...
// Package registry provides the ExtensionRegistry interface and implementation
// for managing Runtime SDK extension registrations.
//
// Copied from sigs.k8s.io/cluster-api/internal/runtime/registry.
...
func (r *extensionRegistry) Add(extensionConfig *runtimev1.ExtensionConfig) error {
	...
	if !r.ready {
		return errors.Errorf(
			"failed to add ExtensionConfig %q to registry: invalid operation: Add cannot be called on a registry which has not been warmed up",
...
func (r *extensionRegistry) Remove(extensionConfig *runtimev1.ExtensionConfig) error {
	...
	if !r.ready {
		return errors.Errorf(
			"failed to remove ExtensionConfig %q from registry: invalid operation: Remove cannot be called on a registry which has not been warmed up",
			extensionConfig.Name,
		)
	}
...
func (r *extensionRegistry) List(gh runtimecatalog.GroupHook) ([]*ExtensionRegistration, error) {
	...
	if !r.ready {
		return nil, errors.Errorf(
			"failed to list extension handlers for GroupHook %q: invalid operation: List cannot be called on a registry which has not been warmed up",
			gh.String(),
		)
	}
...
func (r *extensionRegistry) Get(name string) (*ExtensionRegistration, error) {
	...
	if !r.ready {
		return nil, errors.Errorf(
			"failed to get extension handler %q from registry: invalid operation: Get cannot be called on a registry not yet ready",
			name,
		)
	}
...
func (r *extensionRegistry) add(extensionConfig *runtimev1.ExtensionConfig) error {
	...
	for _, e := range extensionConfig.Status.Handlers {
		...
		if err != nil {
			allErrs = append(allErrs, errors.Wrapf(err,
				"failed to add extension handler %q to registry: failed to parse GroupVersion %q of handler %q",
				e.Name, e.RequestHook.APIVersion, e.Name))
