...
// upstream CAPI, so an external provider like CAPRKE2 can't directly consume it.
// We duplicate the necessary functions to interact with the Kubernetes API and
// fetch APIVersions and address the move from ObjectReference -> ContractVersionedObjectReference.

// Version is the contract version supported by the current upstream Cluster API version.
...
var Version = clusterv1.GroupVersion.Version
