...
| Name        | Description                                                        | Default | Env Variable       |
|-------------|--------------------------------------------------------------------|---------|--------------------|
| namespace   | Controller watches this namespace to reconcile cluster-api objects | ""      | WATCH_NAMESPACE    | 
...
- Configure the namespace and concurrency options for manager:
> **Note:** Enabling/Disabling feature gates is supported for `rke2-bootstrap-controller-manager` as well as `rke2-control-plane-controller-manager`.
- Update `namespace` arg:
```shell
$ kubectl -n system set env deployment/controller-manager WATCH_NAMESPACE=<preferred-namespace>
```
