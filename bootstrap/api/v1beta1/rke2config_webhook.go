...
func (s *RKE2ConfigSpec) validateFileSource(pathPrefix *field.Path) field.ErrorList {
	...
	for i, file := range s.Files {
		if file.ContentFrom != nil {
			if file.ContentFrom.ConfigMap != (ConfigMapFileSource{}) && file.ContentFrom.Secret != (SecretFileSource{}) {
				allErrs = append(
					allErrs,
			...
			} else if file.ContentFrom.ConfigMap == (ConfigMapFileSource{}) && file.ContentFrom.Secret == (SecretFileSource{}) {
				allErrs = append(
					allErrs,
					field.Invalid(pathPrefix.Child("files").Index(i).Child("contentFrom"),
						file,
