...
import (
	...
	"github.com/rancher/cluster-api-provider-rke2/pkg/capi/compare"
	"github.com/rancher/cluster-api-provider-rke2/pkg/capi/inplace"
	patchutil "github.com/rancher/cluster-api-provider-rke2/pkg/capi/patchutil"
	"github.com/rancher/cluster-api-provider-rke2/pkg/rke2"
)

func (r *RKE2ControlPlaneReconciler) canUpdateMachine(
	ctx context.Context, machine *clusterv1.Machine, machineUpToDateResult rke2.UpToDateResult,
) (bool, error) {
	...
	if machineUpToDateResult.DesiredMachine == nil ||
		...
		machineUpToDateResult.CurrentRKE2Config == nil ||
		machineUpToDateResult.DesiredRKE2Config == nil {
		return false, nil
	}
...
func (r *RKE2ControlPlaneReconciler) canExtensionsUpdateMachine(
	ctx context.Context, machine *clusterv1.Machine,
	machineUpToDateResult rke2.UpToDateResult, extensionHandlers []string,
) (bool, []string, error) {
	log := ctrl.LoggerFrom(ctx)

	// Create the CanUpdateMachine request.
	req, err := createCanUpdateRequest(ctx, r.Client, machine, machineUpToDateResult)
	if err != nil {
	...
	for _, extensionHandler := range extensionHandlers {
		...
		log.V(5).Info(
			fmt.Sprintf("Machine cannot be updated in-place yet after calling extension %s: %s",
				extensionHandler, strings.Join(reasons, ",")),
			"Machine", klog.KObj(&req.Current.Machine),
		)
	}
...
func createCanUpdateRequest(
	ctx context.Context, c client.Client, currentMachine *clusterv1.Machine,
	...
	currentRKE2ConfigForDiff := machineUpToDateResult.CurrentRKE2Config.DeepCopy()
	...
	desiredRKE2ConfigForDiff := machineUpToDateResult.DesiredRKE2Config.DeepCopy()
	desiredInfraMachineForDiff := machineUpToDateResult.DesiredInfraMachine.DeepCopy()

	// Sync in-place mutable changes from desired to current RKE2Config / InfraMachine.
	// Note: Writing these fields is handled by syncMachines and not the responsibility of in-place updates.
	// Note: Desired RKE2Config / InfraMachine already contain the latest labels & annotations.
	currentRKE2ConfigForDiff.SetLabels(desiredRKE2ConfigForDiff.GetLabels())
	currentRKE2ConfigForDiff.SetAnnotations(desiredRKE2ConfigForDiff.GetAnnotations())
	...
	// Apply defaulting to desired Machine / InfraMachine via SSA dry-run.
	// Note: currentMachineForDiff doesn't need a dry-run as it was just written in syncMachines and then
	//       updated in controlPlane to ensure the Machine we get here is the latest version of the Machine.
	...
	if err := ssa.Patch(ctx, c, rke2ManagerName, desiredMachineForDiff, ssa.WithDryRun{}); err != nil {
	...
	if err := ssa.Patch(ctx, c, rke2ManagerName, currentInfraMachineForDiff, ssa.WithDryRun{}); err != nil {
		return nil, errors.Wrap(err, "server side apply dry-run failed for current InfraMachine")
	}

	if err := ssa.Patch(ctx, c, rke2ManagerName, desiredInfraMachineForDiff, ssa.WithDryRun{}); err != nil {
	...
	req.Current.BootstrapConfig, err = patchutil.ConvertToRawExtension(cleanupRKE2Config(currentRKE2ConfigForDiff))
	if err != nil {
		return nil, err
	}

	req.Desired.BootstrapConfig, err = patchutil.ConvertToRawExtension(cleanupRKE2Config(desiredRKE2ConfigForDiff))
	if err != nil {
		return nil, err
	}

	req.Current.InfrastructureMachine, err = patchutil.ConvertToRawExtension(cleanupUnstructured(currentInfraMachineForDiff))
	if err != nil {
		return nil, err
	}

	req.Desired.InfrastructureMachine, err = patchutil.ConvertToRawExtension(cleanupUnstructured(desiredInfraMachineForDiff))
	if err != nil {
		return nil, err
	}
...
func cleanupRKE2Config(config *bootstrapv1.RKE2Config) *bootstrapv1.RKE2Config {
...
func applyPatchesToRequest(ctx context.Context, req *runtimehooksv1.CanUpdateMachineRequest, resp *runtimehooksv1.CanUpdateMachineResponse) error {
	if resp.MachinePatch.IsDefined() {
		if err := patchutil.ApplyPatchToTypedObject(ctx, &req.Current.Machine, resp.MachinePatch, "spec"); err != nil {
			return err
		}
	}

	if resp.BootstrapConfigPatch.IsDefined() {
		if _, err := patchutil.ApplyPatchToObject(ctx, &req.Current.BootstrapConfig, resp.BootstrapConfigPatch, "spec"); err != nil {
			return err
		}
	}

	if resp.InfrastructureMachinePatch.IsDefined() {
		if _, err := patchutil.ApplyPatchToObject(ctx, &req.Current.InfrastructureMachine, resp.InfrastructureMachinePatch, "spec"); err != nil {
			return err
		}
	}
...
func matchesMachine(req *runtimehooksv1.CanUpdateMachineRequest) (bool, []string, error) {
	...
	if err != nil {
		return false, nil, errors.Wrapf(err, "failed to match RKE2Config")
	}

	if !match {
		reasons = append(reasons, "RKE2Config cannot be updated in-place: "+diff)
	}
	...
	if !match {
		reasons = append(reasons, fmt.Sprintf("%s cannot be updated in-place: %s",
			req.Current.InfrastructureMachine.Object.GetObjectKind().GroupVersionKind().Kind, diff))
	}
...
func matchesUnstructuredSpec(patched, desired runtime.RawExtension) (equal bool, diff string, matchErr error) {
	// Note: Both patched and desired objects are always Unstructured as createCanUpdateRequest and
