...
func (r *RKE2ControlPlaneReconciler) reconcileUnhealthyMachines(ctx context.Context, controlPlane *rke2.ControlPlane) (ret ctrl.Result, retErr error) {
	...
	if controlPlane.RCP.Status.Initialized {
		...
		if controlPlane.Machines.Len() <= 1 {
			...
			conditions.MarkFalse(machineToBeRemediated, clusterv1.MachineOwnerRemediatedCondition, clusterv1.WaitingForRemediationReason, clusterv1.ConditionSeverityWarning, "RKE2ControlPlane can't remediate if current replicas are less or equal to 1")
		...
		if controlPlane.HasHealthyMachineStillProvisioning() {
			...
			conditions.MarkFalse(machineToBeRemediated, clusterv1.MachineOwnerRemediatedCondition, clusterv1.WaitingForRemediationReason, clusterv1.ConditionSeverityWarning, "RKE2ControlPlane waiting for control plane machine provisioning to complete before triggering remediation")
		...
		if controlPlane.HasDeletingMachine() {
			...
			conditions.MarkFalse(machineToBeRemediated, clusterv1.MachineOwnerRemediatedCondition, clusterv1.WaitingForRemediationReason, clusterv1.ConditionSeverityWarning, "RKE2ControlPlane waiting for control plane machine deletion to complete before triggering remediation")
		...
		if controlPlane.IsEtcdManaged() {
			...
			if !canSafelyRemediate {
				...
				conditions.MarkFalse(machineToBeRemediated, clusterv1.MachineOwnerRemediatedCondition, clusterv1.WaitingForRemediationReason, clusterv1.ConditionSeverityWarning, "RKE2ControlPlane can't remediate this machine because this could result in etcd loosing quorum")
...
func (r *RKE2ControlPlaneReconciler) checkRetryLimits(log logr.Logger, machineToBeRemediated *clusterv1.Machine, controlPlane *rke2.ControlPlane, reconciliationTime time.Time) (*RemediationData, bool, error) {
	...
	if lastRemediationTime.Add(retryPeriod).After(reconciliationTime) {
		...
		conditions.MarkFalse(machineToBeRemediated, clusterv1.MachineOwnerRemediatedCondition, clusterv1.WaitingForRemediationReason, clusterv1.ConditionSeverityWarning, "RKE2ControlPlane can't remediate this machine because the operation already failed in the latest %s (RetryPeriod)", retryPeriod)
	...
	if controlPlane.RCP.Spec.RemediationStrategy != nil && controlPlane.RCP.Spec.RemediationStrategy.MaxRetry != nil {
		...
		if remediationInProgressData.RetryCount >= maxRetry {
			...
			conditions.MarkFalse(machineToBeRemediated, clusterv1.MachineOwnerRemediatedCondition, clusterv1.WaitingForRemediationReason, clusterv1.ConditionSeverityWarning, "RKE2ControlPlane can't remediate this machine because the operation already failed %d times (MaxRetry)", maxRetry)
...
func (r *RKE2ControlPlaneReconciler) canSafelyRemoveEtcdMember(ctx context.Context, controlPlane *rke2.ControlPlane, machineToBeRemediated *clusterv1.Machine) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
