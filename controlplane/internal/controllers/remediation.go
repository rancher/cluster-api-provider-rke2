...
func (r *RKE2ControlPlaneReconciler) reconcileUnhealthyMachines(ctx context.Context, controlPlane *rke2.ControlPlane) (ret ctrl.Result, retErr error) { // nolint:gocyclo
	...
	for _, m := range controlPlane.Machines {
		...
		shouldCleanup := v1beta1conditions.IsTrue(m, clusterv1.MachineHealthCheckSucceededCondition) &&
			v1beta1conditions.IsFalse(m, clusterv1.MachineOwnerRemediatedCondition)
		...
		if err := patchHelper.Patch(ctx, m, v1beta1patch.WithOwnedV1Beta1Conditions{Conditions: []clusterv1beta1.ConditionType{
	...
	patchHelper, err := v1beta1patch.NewHelper(machineToBeRemediated, r.Client)
	if err != nil {
		return ctrl.Result{}, err
	}
	...
	defer func() {
		...
		if err := patchHelper.Patch(ctx, machineToBeRemediated, v1beta1patch.WithOwnedV1Beta1Conditions{Conditions: []clusterv1beta1.ConditionType{
			clusterv1.MachineOwnerRemediatedCondition,
		}}); err != nil {
	...
	if controlPlane.RCP.Status.Initialized {
		...
		if controlPlane.Machines.Len() <= 1 {
			...
			v1beta1conditions.MarkFalse(
				...
				clusterv1beta1.MachineOwnerRemediatedCondition,
				clusterv1beta1.WaitingForRemediationReason,
...
func getMachineToBeRemediated(unhealthyMachines collections.Machines, isEtcdManaged bool) *clusterv1beta1.Machine {
...
func pickMachineToBeRemediatedByConditionState(i, j *clusterv1.Machine, t clusterv1beta1.ConditionType) *bool {
...
func (r *RKE2ControlPlaneReconciler) canSafelyRemoveEtcdMember(ctx context.Context,
	...
	for _, etcdMember := range etcdMembers {
		...
		if !v1beta1conditions.IsTrue(machine, controlplanev1.MachineEtcdMemberHealthyCondition) {
			targetUnhealthyMembers++
